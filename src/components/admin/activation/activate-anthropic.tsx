...
import { checkAnthropicHealth } from '@/actions/setup'
import type { HealthCheckResult } from '@/types'
import { CheckCircle2, AlertCircle, Loader2, ExternalLink, Bot } from 'lucide-react'

export function ActivateAnthropic() {
  ...
  async function handleCheck() {
    ...
    try {
      ...
      setHealth({ service: 'anthropic', status: 'error', message: 'Check failed' })
    } finally {
      setIsChecking(false)
    }
  }
  ...
  return (
    <div className="rounded-lg border bg-white p-5">
      <div className="flex items-center gap-3">
        ...
        <h3 className="font-medium text-gray-900">Anthropic — AI Agents</h3>
      ...
      <div className="mt-3 rounded-md bg-gray-50 p-3">
        ...
        <p className="mt-1.5 text-xs text-gray-600">
          <code className="rounded bg-gray-200 px-1 font-mono">ANTHROPIC_API_KEY</code>
        </p>
        <a
      ...
      {health?.status === 'not_configured' && (
        <div className="mt-3 flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 p-3">
          ...
          <p className="text-xs text-amber-700">
            Set ANTHROPIC_API_KEY in your environment variables, then redeploy.
          </p>
        </div>
      )}
