...
export async function POST(request: NextRequest) {
  ...
  // 1. Cold leads: new leads older than 7 days → mark cold
  ...
  // Get all students who have at least one progress record
  ...
  // Stalled = has progress records but none recent
  ...
  // 4. Testimonial milestone check: users who completed 50%+ of a course
  let testimonialRequestsSent = 0
  try {
    // Get all published lessons grouped by module
    ...
    if (allLessons && allLessons.length > 0) {
      ...
      // Get completed lesson progress — only for published lessons
      ...
      if (completedProgress && completedProgress.length > 0) {
        ...
        // Find users who completed 50%+ of any module
        ...
        if (qualifiedUserIds.length > 0) {
          // Filter out users who already have a testimonial request
  ...
  if ((coldLeads ?? 0) > 0) findings.push(`${coldLeads} cold leads`)
